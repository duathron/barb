...
### Added
- **Allowlist staleness warning (B2).** Offline banner-level hint on stderr when the effective Tranco allowlist (user override `~/.barb/data/allowlist.json` if present, else bundled) is older than N days (default 90). Mirrors the `version_check.py` pattern: offline-only (file mtime, no network), opt-outable via `allowlist_check.enabled: false` in `~/.barb/config.yaml`, never blocks analysis, goes to stderr only. New config section `allowlist_check` with fields `enabled` (bool, default `true`) and `max_age_days` (int, default `90`).
- **Aggregated batch summary + `--summary-only` (B3).** For N>1 URLs, rich and console output now opens with an aggregated header block: verdict histogram (count + share per tier), top 5 signals across all URLs, and share of URLs exceeding `--threshold`. New `--summary-only` flag suppresses per-URL detail blocks (keeps the aggregate block and the existing per-URL table). Console format (`-o console`) also gains the aggregate header for the first time. **JSON/NDJSON/CSV/pipe output shapes are completely unchanged** — the aggregate is a presentation-layer feature only. Single-URL path is unchanged (N=1 produces no aggregate block).
